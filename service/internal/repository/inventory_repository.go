...
func (r *InventoryRepository) GetInventories(ctx *fiber.Ctx, filters map[string]string, span opentracing.Span) ([]dtos.InventoryListDTO, int, error) {
	...
	args, i, condition, queryGlobal, _, _, err := utils.GetInventoryCondition(ctx, filters, childSpan)
	if err != nil {
    ...
    FROM ( 
        ...
        FROM inventories iv
				LEFT JOIN inv_dts ivd ON ivd.inventory_id = iv.id AND ivd.deleted_at IS NULL
				...
				LEFT JOIN purchase_orders po ON po.id                  = ivd.inventory_id
...
func (r *InventoryRepository) GetInventoryDetails(ctx *fiber.Ctx, filters map[string]string, span opentracing.Span) ([]dtos.InventoryDetailDTO, int, error) {
	childSpan := opentracing.StartSpan("InventoryRepository-GetInventoryDetails", opentracing.ChildOf(span.Context()))
	...
	args, i, condition, queryGlobal, joinCondition, customCondition, err := utils.GetInventoryCondition(ctx, filters, childSpan)
	if err != nil {
		utils.LogErrors(childSpan, err)
		return nil, 0, err
	}
...
func (r *InventoryRepository) GetInventoryDetailsDts(ctx *fiber.Ctx, filters map[string]string, salesOrderIDs []uint, span opentracing.Span) ([]dtos.InventoryInvDtListDTO, int, error) {
	childSpan := opentracing.StartSpan("InventoryRepository-GetInventoryDetailsDts", opentracing.ChildOf(span.Context()))
	...
	products := []dtos.InventoryInvDtListDTO{}
	...
	if len(salesOrderIDs) > 0 {
		condition += fmt.Sprintf(" AND ivd.inventory_id = ANY($%d)", i)
    ...
    FROM ( 
        SELECT DISTINCT ON (ivd.id)
						ivd.id, ivd.inventory_id, ivd.product_uuid,
						ivd.item_unit_id, ivd.vat_id, ivd.item_id, ivd.ref_type, ivd.item_type, ivd.gen_code, ivd.remark, ivd.vat_perc, ivd.qty_out, ivd.qty_invoice, ivd.qty, ivd.price_sell, ivd.price_buy, ivd.subtotal_sell, ivd.subtotal_buy, ivd.total_am, ivd.created_by_id, ivd.updated_by_id, ivd.deleted_by_id, ivd.created_at, ivd.updated_at, ivd.deleted_at,
						ivd.vat_perc, ivd.vat_perc_am, ivd.pph23_perc, ivd.pph23_perc_am, ivd.is_vat, ivd.is_pph23,
						ivd.created_at, ivd.updated_at, ivd.deleted_at,
						TO_CHAR(ivd.expired_at, 'YYYY-MM-DD') as expired_at,
					...
					FROM inv_dts ivd
					LEFT JOIN inventories iv ON ivd.inventory_id = iv.id AND iv.deleted_at IS NULL
					LEFT JOIN so_dts sd ON sd.id = ivd.ref_so_dt_id AND ivd.ref_type = 'so'
					LEFT JOIN so_dt_boms sdb ON sdb.id = ivd.ref_so_dt_bom_id AND ivd.ref_type = 'so'
					LEFT JOIN request_order_dts rd ON rd.id = ivd.ref_ro_dt_id
					LEFT JOIN request_orders ro ON ro.id = rd.request_order_id
