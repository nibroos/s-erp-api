...
func (r *InvoiceDpRepository) GetInvoiceDps(ctx *fiber.Ctx, filters map[string]string, span opentracing.Span) ([]dtos.InvoiceDpListDTO, int, error) {
	...
	args, i, condition, queryGlobal, _, _, err := utils.GetInvoiceDpCondition(ctx, filters, childSpan)
	if err != nil {
    ...
    FROM ( 
        SELECT DISTINCT ON (idp.id)
				...
				LEFT JOIN invoice_dp_dts sidt ON sidt.invoice_dp_id = idp.id
...
func (r *InvoiceDpRepository) GetInvoiceDpsDetails(ctx *fiber.Ctx, filters map[string]string, span opentracing.Span) ([]dtos.InvoiceDpDetailDTO, int, error) {
	childSpan := opentracing.StartSpan("InvoiceDpRepository-GetInvoiceDpsDetails", opentracing.ChildOf(span.Context()))
	...
	products := []dtos.InvoiceDpDetailDTO{}

	var total int

	args, i, condition, queryGlobal, joinCondition, customCondition, err := utils.GetInvoiceDpCondition(ctx, filters, childSpan)
	if err != nil {
		utils.LogErrors(childSpan, err)
		return nil, 0, err
	}
...
func (r *InvoiceDpRepository) GetInvoiceDpDetailsDts(ctx *fiber.Ctx, filters map[string]string, salesOrderIDs []uint, span opentracing.Span) ([]dtos.InvoiceDpDtListDTO, int, error) {
	childSpan := opentracing.StartSpan("InvoiceDpRepository-GetInvoiceDpDetailsDts", opentracing.ChildOf(span.Context()))
	...
	products := []dtos.InvoiceDpDtListDTO{}
	...
	if len(salesOrderIDs) > 0 {
		condition += fmt.Sprintf(" AND sidt.invoice_dp_id = ANY($%d)", i)
    ...
    FROM ( 
        SELECT DISTINCT ON (sidt.id)
					sidt.id, sidt.product_uuid, sidt.invoice_dp_id, sidt.item_unit_id, sidt.vat_id, sidt.pph23_id, 
					sidt.ref_id, sidt.ref_dt_id, sidt.product_id, sidt.product_id as item_id, sidt.ref_type, sidt.product_type, sidt.remark, 
					sidt.dp_percentage, sidt.is_vat, sidt.is_pph23, sidt.qty, sidt.price, sidt.subtotal,
					sidt.discount, sidt.total_amount, sidt.total_dp, sidt.created_by_id, sidt.updated_by_id, sidt.deleted_by_id, 
				...
				LEFT JOIN invoice_dps idp ON sidt.invoice_dp_id = idp.id
				LEFT JOIN customers c ON idp.customer_id = c.id
				LEFT JOIN mix_values cur ON idp.currency_id = cur.id
				LEFT JOIN mix_values pt ON idp.payment_term_id = pt.id
				LEFT JOIN mix_values vat ON idp.vat_id = vat.id
...
func (r *InvoiceDpRepository) GetInvoiceDpDetailsDtBoms(ctx *fiber.Ctx, filters map[string]string, salesOrderIDs []uint, span opentracing.Span) ([]dtos.SalesOrderSoDtBomListDTO, int, error) {
	childSpan := opentracing.StartSpan("InvoiceDpRepository-GetInvoiceDpDetailsDtBoms", opentracing.ChildOf(span.Context()))
	...
	if len(salesOrderIDs) > 0 {
		condition += fmt.Sprintf(" AND idp.id = ANY($%d)", i)
		args = append(args, pq.Array(salesOrderIDs))
		i++
	}
    ...
    FROM ( 
        ...
        FROM so_dt_boms sdb
				...
				LEFT JOIN invoice_dp_dts sidt ON sd.id = sidt.ref_dt_id AND sidt.ref_type = 'so' 
				LEFT JOIN products p ON sidt.product_id = p.id
				LEFT JOIN products it ON it.id = sdb.item_id
...
func (r *InvoiceDpRepository) GetWidgetInvoiceDps(ctx *fiber.Ctx, filters map[string]string, span opentracing.Span) ([]dtos.InvoiceDpStatusWidget, int, error) {
	...
	args, _, condition, queryGlobal, _, _, err := utils.GetInvoiceDpCondition(ctx, filters, childSpan)
	if err != nil {
		utils.LogErrors(childSpan, err)
		return nil, 0, err
	}
