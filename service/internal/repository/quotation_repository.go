...
func (r *QuotationRepository) GetQuotations(ctx *fiber.Ctx, filters map[string]string, span opentracing.Span) ([]dtos.QuotationListDTO, int, error) {
	...
	args, i, condition, queryGlobal, err := utils.GetQuotationCondition(ctx, filters, childSpan)
	if err != nil {
    ...
    FROM ( 
        SELECT DISTINCT ON (q.id)
					q.id, q.customer_id, q.order_type_id, q.currency_id, q.vat_id, q.payment_id, q.pph23_id, q.branch_id, q.quo_no, q.title, q.remark, q.status, q.exchange_rate, q.pph23_perc, q.markup_perc, q.total_qty, q.subtotal, q.total_discount, q.total_pph23, q.total_vat, q.grand_total, q.created_by_id, q.updated_by_id, q.deleted_by_id, q.created_at, q.updated_at, q.deleted_at,
	...
	orderColumn := utils.GetStringOrDefault(filters["order_column"], "due_at")
...
func (r *QuotationRepository) GetQuotationDetails(ctx *fiber.Ctx, filters map[string]string, span opentracing.Span) ([]dtos.QuotationDetailDTO, int, error) {
	childSpan := opentracing.StartSpan("QuotationRepository-GetQuotationDetails", opentracing.ChildOf(span.Context()))
	...
	products := []dtos.QuotationDetailDTO{}
    ...
    FROM ( 
        ...
        LEFT JOIN users uu ON qd.updated_by_id = uu.id
				WHERE 1=1` + condition + queryGlobal + `
				AND q.deleted_at IS NULL AND qd.deleted_at IS NULL AND qdb.deleted_at IS NULL
    ) AS alias WHERE 1=1`
...
func (r *QuotationRepository) GetQuotationDetailsDts(ctx *fiber.Ctx, filters map[string]string, quotationIDs []uint, span opentracing.Span) ([]dtos.QuotationQuoDtListDTO, error) {
	childSpan := opentracing.StartSpan("QuotationRepository-GetQuotationDetailsDts", opentracing.ChildOf(span.Context()))
	...
	products := []dtos.QuotationQuoDtListDTO{}
	...
	if len(quotationIDs) > 0 {
		condition += fmt.Sprintf(" AND qd.quotation_id = ANY($%d)", i)
...
func (r *QuotationRepository) GetQuotationDetailsDtBoms(ctx *fiber.Ctx, filters map[string]string, quotationIDs []uint, span opentracing.Span) ([]dtos.QuotationQuoDtBomListDTO, error) {
	childSpan := opentracing.StartSpan("QuotationRepository-GetQuotationDetailsDtBoms", opentracing.ChildOf(span.Context()))
	...
	products := []dtos.QuotationQuoDtBomListDTO{}
	...
	if len(quotationIDs) > 0 {
		// Convert quotationIDs to a string for SQL IN clause
		// use ANY clause for PostgreSQL
    ...
    FROM ( 
        ...
        LEFT JOIN users cu ON qdb.created_by_id = cu.id
        LEFT JOIN users uu ON qdb.updated_by_id = uu.id
				WHERE 1=1` + condition + queryGlobal + `
				AND q.deleted_at IS NULL AND qd.deleted_at IS NULL AND qdb.deleted_at IS NULL
    ) AS alias WHERE 1=1`
	...
	log.Println("QuotationRepository-GetQuotationDetailsDtBoms query:", query, products)
	return products, nil
}
...
func (r *QuotationRepository) GetQuotationByID(ctx *fiber.Ctx, params *dtos.GetQuotationParams, tx *gorm.DB, span opentracing.Span) (*dtos.QuotationDetailDTO, error) {
    ...
    FROM ( 
        SELECT DISTINCT ON (q.id)
					q.id, q.customer_id, q.order_type_id, q.currency_id, q.vat_id, q.payment_id, q.pph23_id, q.branch_id, q.quo_no, q.title, q.remark, q.status, q.exchange_rate, q.total_qty, q.subtotal, q.total_discount, q.total_pph23, q.total_vat, q.grand_total, q.created_by_id, q.updated_by_id, q.deleted_by_id, q.created_at, q.updated_at, q.deleted_at,
					TO_CHAR(q.due_at, 'YYYY-MM-DD') as due_at,
					TO_CHAR(q.expired_at, 'YYYY-MM-DD') as expired_at,
