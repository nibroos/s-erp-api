...
func (r *PurchaseOrderRepository) GetPurchaseOrders(ctx *fiber.Ctx, filters map[string]string, span opentracing.Span) ([]dtos.PurchaseOrderListDTO, int, error) {
	...
	args, i, condition, queryGlobal, _, _, err := utils.GetPurchaseOrderCondition(ctx, filters, childSpan)
	if err != nil {
...
func (r *PurchaseOrderRepository) GetPurchaseOrderDetails(ctx *fiber.Ctx, filters map[string]string, span opentracing.Span) ([]dtos.PurchaseOrderDetailDTO, int, error) {
	childSpan := opentracing.StartSpan("PurchaseOrderRepository-GetPurchaseOrderDetails", opentracing.ChildOf(span.Context()))
	...
	products := []dtos.PurchaseOrderDetailDTO{}

	var total int

	args, i, condition, queryGlobal, joinCondition, customCondition, err := utils.GetPurchaseOrderCondition(ctx, filters, childSpan)
	if err != nil {
		utils.LogErrors(childSpan, err)
		return nil, 0, err
	}
...
func (r *PurchaseOrderRepository) GetPurchaseOrderDetailsDts(ctx *fiber.Ctx, filters map[string]string, salesOrderIDs []uint, span opentracing.Span) ([]dtos.PurchaseOrderPoDtListDTO, int, error) {
	childSpan := opentracing.StartSpan("PurchaseOrderRepository-GetPurchaseOrderDetailsDts", opentracing.ChildOf(span.Context()))
	...
	products := []dtos.PurchaseOrderPoDtListDTO{}
	...
	if len(salesOrderIDs) > 0 {
		condition += fmt.Sprintf(" AND pd.po_id = ANY($%d)", i)
    ...
    FROM ( 
        ...
        FROM purchase_order_dts pd
				LEFT JOIN purchase_orders po ON pd.po_id = po.id
				LEFT JOIN so_dts sd ON sd.id = pd.ref_so_dt_id AND pd.ref_type = 'so'
				LEFT JOIN sales_orders so ON (so.id = sd.sales_order_id)
				LEFT JOIN request_order_dts rod ON pd.ref_ro_dt_id = rod.id AND pd.ref_type = 'ro'
				LEFT JOIN products p ON pd.product_id = p.id
