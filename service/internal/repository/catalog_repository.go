...
func NewCatalogRepository(db *gorm.DB, sqlDB *sqlx.DB, tracer opentracing.Tracer) *CatalogRepository {
...
func (r *CatalogRepository) GetCatalogs(ctx *fiber.Ctx, filters map[string]string, span opentracing.Span) ([]dtos.CatalogListDTO, int, error) {
	// Create a child span for the controller
	childSpan := opentracing.StartSpan("CatalogRepository-GetCatalogs", opentracing.ChildOf(span.Context()))
	...
	go func() {
		...
		err := r.sqlDB.SelectContext(ctx.Context(), &catalogs, query, args...)
		if err != nil {
...
func (r *CatalogRepository) GetCatalogByID(ctx *fiber.Ctx, params *dtos.GetCatalogParams, span opentracing.Span) (*dtos.CatalogDetailDTO, error) {
	childSpan := opentracing.StartSpan("CatalogRepository-GetCatalogByID", opentracing.ChildOf(span.Context()))
...
func (r *CatalogRepository) Rollback() *gorm.DB {
	return r.db.Rollback()
}

func (r *CatalogRepository) CreateCatalog(tx *gorm.DB, catalog *models.Catalog, span opentracing.Span) error {
	childSpan := opentracing.StartSpan("CatalogRepository-CreateCatalog", opentracing.ChildOf(span.Context()))
...
func (r *CatalogRepository) UpdateCatalog(tx *gorm.DB, catalog *models.Catalog, span opentracing.Span) error {
	childSpan := opentracing.StartSpan("CatalogRepository-UpdateCatalog", opentracing.ChildOf(span.Context()))

	if err := tx.Select("*").Omit(
		"created_at", "created_by_id", "branch_id",
	).Updates(catalog).Error; err != nil {
		utils.LogErrors(childSpan, err)
		return err
	}
	return nil
...
func (r *CatalogRepository) DeleteCatalog(tx *gorm.DB, params *dtos.GetCatalogParams, span opentracing.Span) error {
	childSpan := opentracing.StartSpan("CatalogRepository-DeleteCatalog", opentracing.ChildOf(span.Context()))

	if err := tx.Delete(&models.Catalog{}, params.ID).Error; err != nil {
		utils.LogErrors(childSpan, err)
		return err
	}
	return nil
...
func (s *CatalogRepository) RestoreCatalog(tx *gorm.DB, params *dtos.GetCatalogParams, span opentracing.Span) error {
	childSpan := opentracing.StartSpan("CatalogRepository-RestoreCatalog", opentracing.ChildOf(span.Context()))

	var catalog models.Catalog
	if err := tx.Unscoped().Model(&catalog).Where("id = ?", params.ID).Update("deleted_at", nil).Error; err != nil {
		utils.LogErrors(childSpan, err)
		return err
	}
	return nil
...
func (r *CatalogRepository) CreateBoms(tx *gorm.DB, boms []*models.Bom, catalogID uint, span opentracing.Span) error {
