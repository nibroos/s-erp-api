...
func (r *SalesInvoiceRepository) GetSalesInvoicesDetails(ctx *fiber.Ctx, filters map[string]string, span opentracing.Span) ([]dtos.SalesInvoiceDetailDTO, int, error) {
	childSpan := opentracing.StartSpan("SalesInvoiceRepository-GetSalesInvoicesDetails", opentracing.ChildOf(span.Context()))
	...
	products := []dtos.SalesInvoiceDetailDTO{}

	var total int

	args, i, condition, queryGlobal, joinCondition, customCondition, err := utils.GetSalesInvoiceCondition(ctx, filters, childSpan)
	if err != nil {
		utils.LogErrors(childSpan, err)
		return nil, 0, err
	}
...
func (r *SalesInvoiceRepository) GetSalesInvoiceDetailsDts(ctx *fiber.Ctx, filters map[string]string, salesOrderIDs []uint, span opentracing.Span) ([]dtos.SalesInvoiceDtListDTO, int, error) {
	childSpan := opentracing.StartSpan("SalesInvoiceRepository-GetSalesInvoiceDetailsDts", opentracing.ChildOf(span.Context()))
	...
	products := []dtos.SalesInvoiceDtListDTO{}
	...
	if len(salesOrderIDs) > 0 {
		condition += fmt.Sprintf(" AND sidt.sales_invoice_id = ANY($%d)", i)
    ...
    FROM ( 
        SELECT DISTINCT ON (sidt.id)
				...
				LEFT JOIN sales_invoices si ON sidt.sales_invoice_id = si.id
				LEFT JOIN customers c ON si.customer_id = c.id
				LEFT JOIN mix_values cur ON si.currency_id = cur.id
				LEFT JOIN mix_values pt ON si.payment_term_id = pt.id
				LEFT JOIN mix_values vat ON si.vat_id = vat.id
...
func (r *SalesInvoiceRepository) GetSalesInvoiceDetailsDtBoms(ctx *fiber.Ctx, filters map[string]string, salesOrderIDs []uint, span opentracing.Span) ([]dtos.SalesOrderSoDtBomListDTO, int, error) {
	childSpan := opentracing.StartSpan("SalesInvoiceRepository-GetSalesInvoiceDetailsDtBoms", opentracing.ChildOf(span.Context()))
	...
	if len(salesOrderIDs) > 0 {
		condition += fmt.Sprintf(" AND si.id = ANY($%d)", i)
		args = append(args, pq.Array(salesOrderIDs))
		i++
	}
    ...
    FROM ( 
        ...
        FROM so_dt_boms sdb
				...
				LEFT JOIN sales_invoice_dts sidt ON sd.id = sidt.ref_dt_id AND sidt.ref_type = 'so' 
				LEFT JOIN products p ON sidt.product_id = p.id
				LEFT JOIN products it ON it.id = sdb.item_id
				...
				-- LEFT JOIN so_dts sd ON sdb.so_dt_id = sd.id
				-- LEFT JOIN sales_invoice_dts sidt ON sd.id = sidt.ref_dt_id AND sd.ref_type = 'so' 
				-- LEFT JOIN sales_invoices si ON sidt.sales_invoice_id = si.id
	...
	log.Println("result products:", len(products))
