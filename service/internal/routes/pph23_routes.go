...
func SetupPph23Routes(pph23s fiber.Router, gormDB *gorm.DB, sqlDB *sqlx.DB, utilRepo *repository.UtilRepository) {
	pph23Repo := repository.NewPph23Repository(gormDB, sqlDB)
	pph23Service := service.NewPph23Service(pph23Repo, utilRepo)
	pph23Controller := rest.NewPph23Controller(pph23Service, pph23Repo)
